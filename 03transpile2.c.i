...
string make_define_var(sType* type, char* name, sInfo* info, bool in_header=false)
{
    ...
    if(type->mClass->mName === "lambda") {
        ...
        for(var it = (type->mArrayNum).begin(); !(type->mArrayNum).end(); it = (type->mArrayNum).next()) {
            if(!it.compile->(info)) {
...
# 716 "03transpile2.c"
                       ;
    vsnprintf(msg2, 2048, code, args);
    
# 718 "03transpile2.c" 3 4
   __builtin_va_end(
# 718 "03transpile2.c"
   args
# 718 "03transpile2.c" 3 4
