...
sType*% solve_generics(sType* type, sType* generics_type, sInfo* info)
{
    ...
    if(klass->mName === "lambda") {
        ...
        for(var it = (type->mParamTypes).begin(); !(type->mParamTypes).end(); it = (type->mParamTypes).next()) {
        ...
        for(var it = (type->mGenericsTypes).begin(); !(type->mGenericsTypes).end(); it = (type->mGenericsTypes).next()) {
...
{
    int i = 0;
    for(var it = (info->right_value_objects).begin(); !(info->right_value_objects).end(); it = (info->right_value_objects).next()) {
...
# 255 "04heap.c" 3 4
                               stderr
# 255 "04heap.c"
                                     , "%s %d: can't create generics finalizer\n", info->sname, info->sline);
                        exit(2);
                    }
                    finalizer = info->funcs[fun_name2];
                }
...
# 279 "04heap.c" 3 4
                       ((void *)0) 
# 279 "04heap.c"
                            && !type->mClass->mProtocol && !type->mClass->mNumber
            && gComelang)
        {
...
{
    var right_value_objects = info->right_value_objects;
    for(var it = (right_value_objects).begin(); !(right_value_objects).end(); it = (right_value_objects).next()) {
        if(it && !it->mFreed) {
...
void free_objects(sVarTable* table, sVar* ret_value, sInfo* info)
{
    for(var it = (table->mVars).begin(); !(table->mVars).end(); it = (table->mVars).next()) {
