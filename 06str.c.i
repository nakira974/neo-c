...
bool sListNode*::compile(sListNode* self, sInfo* info)
{
    ...
    for(var it = (list_elements).begin(); !(list_elements).end(); it = (list_elements).next()) {
        if(!it.compile->(info)) {
            return false;
        }
...
bool sTupleNode*::compile(sTupleNode* self, sInfo* info)
{
    ...
    for(var it = (tuple_elements).begin(); !(tuple_elements).end(); it = (tuple_elements).next()) {
        if(!it.compile->(info)) {
            return false;
        }
    ...
    for(var it = (tuple_types).begin(); !(tuple_types).end(); it = (tuple_types).next()) {
    ...
    int i = 1;
    for(var it = (tuple_values).begin(); !(tuple_values).end(); it = (tuple_values).next()) {
            ...
            if(xisdigit(*info->p)) {
                ...
                switch(*info->p) {
                    ...
                    case 'X': {
                        ...
                        unsigned long long int n = strtoll(buf, 
# 906 "06str.c" 3 4
                                                               ((void *)0)
# 906 "06str.c"
                                                                   , 0);
        ...
        while(1) {
            if(*info->p == '"') {
                ...
                if(xisdigit(*info->p)) {
                    ...
                    unsigned long lont int n = strtoll(buf, 
# 1124 "06str.c" 3 4
                                                           ((void *)0)
# 1124 "06str.c"
                                                               , 0);
