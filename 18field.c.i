...
bool sStoreFieldNode*::compile(sStoreFieldNode* self, sInfo* info)
{
    ...
    klass = info.classes[klass->mName];
    for(var field = (klass->mFields).begin(); !(klass->mFields).end(); field = (klass->mFields).next()) {
...
bool sStoreArrayNode*::compile(sStoreArrayNode* self, sInfo* info)
{
    ...
    for(var it = (array_num_nodes).begin(); !(array_num_nodes).end(); it = (array_num_nodes).next()) {
        if(!it.compile->(info)) {
            return false;
        }
...
# 463 "18field.c"
        ...
        for(var it = (array_num).begin(); !(array_num).end(); it = (array_num).next()) {
