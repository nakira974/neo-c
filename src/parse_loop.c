...
BOOL parse_catch(unsigned int* node, sParserInfo* info)
{
    ...
    if(!add_variable_to_table(info->lv_table, var_names[0], NULL, gNullLVALUE, -1, info->mBlockLevel == 0, FALSE))
    {
    ...
    if(!add_variable_to_table(info->lv_table, var_names[1], NULL, gNullLVALUE, -1, info->mBlockLevel == 0, FALSE))
    {
        fprintf(stderr, "overflow variable table\n");
        exit(2);
    }
